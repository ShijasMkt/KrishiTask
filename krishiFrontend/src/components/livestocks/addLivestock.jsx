...
export default function AddLivestock({ onClose }) {
	...
	const saveLivestock = async(e) => {
    e.preventDefault();
    const token=await getValidAccessToken();
    const body=JSON.stringify({formData});
    const res = await fetch("http://127.0.0.1:8000/api/add_livestock/", {
			method: "POST",
			headers: {
				"Content-Type": "application/json",
				Authorization: `Bearer ${token}`,
    ...
    if (res.ok) {
          Swal.fire({
            ...
            text: "You’ve successfully added a livestock.",
          }).then(onClose());
        } else {
          Swal.fire({
            icon: "error",
            title: "Error",
            text: "Failed to add livestock, please try again.",
          }).then(onClose());
        }
  };
	return (
		<section id="add-livestock">
			<div className="container">
				<form onSubmit={saveLivestock}>
					<div className="row">
